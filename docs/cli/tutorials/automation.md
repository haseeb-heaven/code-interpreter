...
Automate tasks with open-agent. Learn how to use headless mode, pipe data into
open-agent, automate workflows with shell scripts, and generate structured JSON
...
Headless mode runs open-agent once and exits. It's perfect for:
...
Run open-agent in headless mode by providing a prompt with the `-p` (or
...
```bash
openagent -p "Write a poem about TypeScript"
```

## How to pipe input to open-agent

Feed data into open-agent using the standard Unix pipe `|`. open-agent reads the
...
```bash
cat error.log | openagent -p "Explain why this failed"
```

**Windows (PowerShell)**

```powershell
Get-Content error.log | openagent -p "Explain why this failed"
```

Pipe a command:

```bash
git diff | openagent -p "Write a commit message for these changes"
```

## Use open-agent output in scripts

Because open-agent prints to stdout, you can chain it with other tools or save
...
1.  Save the following code as `generate_docs.sh` (or `generate_docs.ps1` for
    ...
    **macOS/Linux (`generate_docs.sh`)**
    ...
    for file in *.py; do
      ...
      # Ask open-agent to generate the documentation and print it to stdout
      openagent -p "Generate a Markdown documentation summary for @$file. Print the
    ...
    **Windows PowerShell (`generate_docs.ps1`)**
    ...
    Get-ChildItem -Filter *.py | ForEach-Object {
      ...
      # Ask open-agent to generate the documentation and print it to stdout
      openagent -p "Generate a Markdown documentation summary for @$($_.Name). Print the result to standard output." | Out-File -FilePath $newName -Encoding utf8
    }
    ```
...
1.  Save the following script as `generate_json.sh` (or `generate_json.ps1` for
    ...
    **macOS/Linux (`generate_json.sh`)**
    ...
    # Extract data
    openagent --output-format json "Return a raw JSON object with keys 'version' and 'deps' from @package.json" | jq -r '.response' > data.json
    ```
    ...
    **Windows PowerShell (`generate_json.ps1`)**
    ...
    $output = openagent --output-format json "Return a raw JSON object with keys 'version' and 'deps' from @package.json" | ConvertFrom-Json
...
2.  Scroll to the very bottom of the file and paste this code:
    ...
    function gcommit() {
      ...
      # Ask open-agent to write the message
      echo "Generating commit message..."
      msg=$(echo "$diff" | openagent -p "Write a concise Conventional Commit message for this diff. Output ONLY the message.")
...
2.  Scroll to the very bottom of the file and paste this code:
    ...
    function gcommit {
      ...
      $msg = $diff | openagent -p "Write a concise Conventional Commit message for this diff. Output ONLY the message."
...
4.  Use your new command:
    ...
    open-agent will analyze your staged changes and commit them with a generated
    message.
