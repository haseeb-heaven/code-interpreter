...
open-agent can send system notifications to alert you when a session completes
...
[Plan Mode](./plan-mode.md), letting you switch to other windows while
open-agent works in the background.
...
notifications, open-agent falls back to a terminal bell (BEL) to get your
...
open-agent sends notifications for the following events:
