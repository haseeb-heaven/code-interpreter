...
Auto Memory is an experimental feature that mines your past open-agent sessions
...
Every session you run with open-agent is recorded locally as a transcript. Auto
...
1.  Open your global settings file at `~/.openagent/settings.json`. If you only
    want Auto Memory in one project, edit `.openagent/settings.json` in that
    project instead.
...
3.  Restart open-agent. The flag requires a restart because the extraction
...
1.  **Eligibility scan.** The service indexes recent sessions from
    `~/.openagent/tmp/<project>/chats/`. Sessions are eligible only if they have
...
6.  **Notification.** When a run produces new candidates, open-agent surfaces an
...
- **Promote** a skill to your user (`~/.openagent/skills/`) or workspace
  (`.openagent/skills/`) directory.
...
- Project or workspace shared instructions in project `OPENAGENT.md` files are
  not auto-extractable. Auto Memory can propose private project memory, global
...
- Explore the [memory management tutorial](./tutorials/memory-management.md) for
  the complementary explicit-memory and `OPENAGENT.md` workflows.
