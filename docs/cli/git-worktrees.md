...
open-agent session its own copy of the codebase. Git worktrees create separate
...
> - Use the **/bug** command within open-agent to file an issue.
...
- **Start with a specific name:** The value you pass becomes both the directory
  name (within `.openagent/worktrees/`) and the branch name.

  ```bash
  openagent --worktree feature-search
  ```

- **Start with a random name:** If you omit the name, open-agent generates a
  random one automatically (for example, `worktree-a1b2c3d4`).
...
When you exit a worktree session (using `/quit` or `Ctrl+C`), open-agent leaves
the worktree intact so your work is not lost. This includes your uncommitted
changes (modified files, staged changes, or untracked files) and any new commits
you have made.

Open-agent prioritizes a fast and safe exit: it **does not automatically
delete** your worktree or branch. You are responsible for cleaning up your
worktrees manually once you are finished with them.

When you exit, open-agent displays instructions on how to resume your work or
how to manually remove the worktree if you no longer need it.

## Resuming work in a Git worktree

To resume a session in a worktree, navigate to the worktree directory and start
open-agent with the `--resume` flag and the session ID:

```bash
cd .openagent/worktrees/feature-search
openagent --resume <session_id>
```
...
- **Clean up a preserved Git worktree:**
  ```bash
  git worktree remove .openagent/worktrees/feature-search --force
...
- **Create a Git worktree manually:**
  ...
  cd ../project-feature-search && openagent
  ```
