...
OpenAgent CLI can integrate with your IDE to provide a more seamless and
...
There are two primary ways to integrate OpenAgent CLI with an IDE:

1.  **VS Code companion extension**: Install the "OpenAgent Companion" extension
    on [Antigravity](https://antigravity.google),
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like OpenAgent CLI.

## VS Code companion extension

The **OpenAgent Companion extension** grants OpenAgent CLI direct access to your
...
- **Native diffing:** When OpenAgent suggests code modifications, you can view
  the changes directly within your IDE's native diff viewer. This lets you
  review, edit, and accept or reject the suggested changes seamlessly.

- **VS Code commands:** You can access OpenAgent CLI features directly from the
  VS Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `OpenAgent: Run`: Starts a new OpenAgent CLI session in the integrated
    terminal.
  - `OpenAgent: Accept Diff`: Accepts the changes in the active diff editor.
  - `OpenAgent: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `OpenAgent: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run OpenAgent CLI inside a supported editor, it will automatically
...
manually, you can run the following command inside OpenAgent CLI:
...
<!-- prettier-ignore -->
> [!NOTE]
> The "OpenAgent Companion" extension may appear towards the bottom of
...
When enabled, OpenAgent CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask OpenAgent to modify a file, it can open a diff view directly in
your editor.
...
- Open the Command Palette and run **OpenAgent: Accept Diff**.
...
- Open the Command Palette and run **OpenAgent: Close Diff Editor**.
...
OpenAgent CLI is officially available in the **ACP Agent Registry**. This allows
you to install and update OpenAgent CLI directly within supporting IDEs and
...
OpenAgent CLI is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install OpenAgent CLI
directly through their in-built registry features.

## Using with sandboxing

If you are using OpenAgent CLI within a sandbox, be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run OpenAgent CLI inside a Docker (or
...
- **Message:**
  ...
  - **Cause:** OpenAgent CLI could not find the necessary environment variables
  ...
  - **Solution:**
    1.  Make sure you have installed the **OpenAgent Companion** extension in
...
If automatic IDE detection fails, or if you are running OpenAgent CLI in a
...
When this variable is set, OpenAgent CLI will skip automatic detection and
attempt to connect using the provided PID.
...
- **Message:**
  ...
  - **Solution:** Run OpenAgent CLI from the integrated terminal of a supported
...
- **Message:**
  ...
  - **Solution:** Open your IDE's extension marketplace, search for "OpenAgent
    Companion", and
