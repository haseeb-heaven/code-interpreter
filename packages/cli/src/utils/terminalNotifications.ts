...
import { debugLogger, writeToStdout } from '@open-agent/core';
...
function sanitizeNotificationContent(
  ...
  return {
    ...
    body: body || 'Open open-agent for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'open-agent needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open open-agent to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'open-agent session complete',
