...
const TEST_OPENAGENT_DIR = path.join(
  os.homedir(),
  OPENAGENT_DIR,
  TMP_DIR_NAME,
  PROJECT_SLUG,
...
const TEST_LOG_FILE_PATH = path.join(TEST_OPENAGENT_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_OPENAGENT_DIR, { recursive: true, force: true });
  } catch {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_OPENAGENT_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_OPENAGENT_DIR);
      expect(
        dirContents.some(
          (f) =>
