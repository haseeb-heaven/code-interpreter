...
class ExecutionSafetyManager:
	...
	_COMPILED_WRITE = tuple(re.compile(p, re.IGNORECASE) for p in _WRITE_PATTERNS)
	_COMPILED_WRITE_ON_HANDLE = tuple(re.compile(p, re.IGNORECASE) for p in _WRITE_ON_HANDLE_PATTERNS)
	...
	def _has_write_operation(self, code: str) -> bool:
		...
		"""
		return any(p.search(code) for p in self._COMPILED_WRITE)
	...
	def _has_write_on_handle(self, code: str) -> bool:
		...
		"""
		return any(p.search(code) for p in self._COMPILED_WRITE_ON_HANDLE)
