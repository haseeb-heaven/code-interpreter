...
class ExecutionSafetyManager:
	...
	# ⚡ Bolt Optimization: Pre-compile regex patterns as class-level attributes.
	# Evaluating strings in `re.search` inside a loop incurs significant overhead due
	# to cache lookups and string parsing. Pre-compiling `re.Pattern` objects provides
	# a reliable speedup in hot paths like `assess_execution`.
	_WRITE_PATTERNS = [
	...
	# ⚡ Bolt Optimization: Extracted inline lists to class properties
	...
	_WIN_DRIVE_COMPILED = re.compile(r"[a-z]:[\\/]")
	_POSIX_ABS_COMPILED = re.compile(r"""["']/[^"'\s]""")
	_OPEN_ARGS_COMPILED = re.compile(r"open\s*\(\s*([\"'][^\"']+[\"'])", re.IGNORECASE)
	_RD_S_Q_COMPILED = re.compile(r"\brd\s+/s\s+/q\b")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._WIN_DRIVE_COMPILED.search(code.lower()):
			return True

		# Quoted POSIX absolute path: '/...' or "/..."
		if self._POSIX_ABS_COMPILED.search(code):
			return True
		...
		open_args = self._OPEN_ARGS_COMPILED.findall(code)
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		if self._RD_S_Q_COMPILED.search(code_lower):
