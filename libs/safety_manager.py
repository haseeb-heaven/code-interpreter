...
class ExecutionSafetyManager:
	...
	_COMPILED_WINDOWS_DRIVE = re.compile(r"[a-z]:[\\/]")
	_COMPILED_QUOTED_POSIX = re.compile(r"""["']/[^"'\s]""")

	_POSIX_SYSTEM_PREFIXES_LIST = [
		r"/etc/\w+",
		r"/tmp/\w+",
		r"/var/\w+",
		r"/usr/\w+",
	...
	_COMPILED_POSIX_SYSTEM_PREFIXES = tuple(re.compile(p, re.IGNORECASE) for p in _POSIX_SYSTEM_PREFIXES_LIST)
	_COMPILED_OPEN_ARGS = re.compile(r"open\s*\(\s*([\"'][^\"']+[\"'])", re.IGNORECASE)
	_COMPILED_WINDOWS_DRIVE_EXACT = re.compile(r"[a-zA-Z]:[\\/]")
	_COMPILED_WINDOWS_RD_DELETE = re.compile(r"\brd\s+/s\s+/q\b")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._COMPILED_WINDOWS_DRIVE.search(code.lower()):
			return True

		# Quoted POSIX absolute path: '/...' or "/..."
		if self._COMPILED_QUOTED_POSIX.search(code):
			return True
		...
		for arg in open_args:
			path = arg.strip("'\"")
			if path.startswith("/") or self._COMPILED_WINDOWS_DRIVE_EXACT.match(path):
				return True
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		if self._COMPILED_WINDOWS_RD_DELETE.search(code_lower):
