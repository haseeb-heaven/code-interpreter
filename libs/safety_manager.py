...
class ExecutionSafetyManager:
	...
	# Pre-compiling these regular expressions at the class level avoids recompiling
	# them on every method call, significantly speeding up the safety checking loops.
	_COMPILED_WRITE_PATTERNS = [re.compile(p, re.IGNORECASE) for p in _WRITE_PATTERNS]
	_COMPILED_WRITE_ON_HANDLE_PATTERNS = [re.compile(p, re.IGNORECASE) for p in _WRITE_ON_HANDLE_PATTERNS]
	_COMPILED_SENSITIVE_POSIX_PREFIXES = [re.compile(p, re.IGNORECASE) for p in _SENSITIVE_POSIX_PREFIXES]
	_COMPILED_DESTRUCTIVE_PATTERNS = [re.compile(p) for p in _DESTRUCTIVE_PATTERNS]
	_COMPILED_SHELL_PATTERNS = [re.compile(p) for p in _SHELL_PATTERNS]

	_COMPILED_WIN_DRIVE_PATH = re.compile(r"[a-z]:[\\/]", re.IGNORECASE)
	_COMPILED_POSIX_ABS_PATH = re.compile(r"""["']/[^"'\s]""")
	_COMPILED_POSIX_SYS_PREFIXES = [
		re.compile(p, re.IGNORECASE) for p in [
			r"/etc/\w+", r"/tmp/\w+", r"/var/\w+", r"/usr/\w+",
			r"/root/\w+", r"/home/\w+/", r"/proc/\w+", r"/sys/\w+",
			r"/dev/\w+", r"/boot/\w+", r"/opt/\w+", r"/mnt/\w+", r"/media/\w+",
		]
	]
	_COMPILED_OPEN_PATH = re.compile(r"open\s*\(\s*([\"'][^\"']+[\"'])", re.IGNORECASE)
	_COMPILED_OPEN_PATH_ABS = re.compile(r"[a-zA-Z]:[\\/]")
	_COMPILED_RECURSIVE_DEL = re.compile(r"\brd\s+/s\s+/q\b")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._COMPILED_WIN_DRIVE_PATH.search(code):
			return True

		# Quoted POSIX absolute path: '/...' or "/..."
		if self._COMPILED_POSIX_ABS_PATH.search(code):
			return True

		# Unquoted well-known POSIX system directory prefixes
		if any(p.search(code) for p in self._COMPILED_POSIX_SYS_PREFIXES):
			return True

		# open() call whose first positional argument is an absolute path string
		open_args = self._COMPILED_OPEN_PATH.findall(code)
		for arg in open_args:
			path = arg.strip("'\"")
			if path.startswith("/") or self._COMPILED_OPEN_PATH_ABS.match(path):
				return True
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		if self._COMPILED_RECURSIVE_DEL.search(code_lower):
