...
class ExecutionSafetyManager:
	...
	_COMPILED_WIN_DRIVE = re.compile(r"[a-z]:[\\/]")
	_COMPILED_POSIX_ABS = re.compile(r"""["']/[^"'\s]""")
	_COMPILED_OPEN_ARGS = re.compile(r"open\s*\(\s*([\"'][^\"']+[\"'])", re.IGNORECASE)

	# Unquoted well-known POSIX system directory prefixes (extracted from _is_host_absolute_path)
	...
	_COMPILED_POSIX_PREFIXES = tuple(re.compile(p, re.IGNORECASE) for p in _POSIX_SYSTEM_PREFIXES)
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if any(p.search(code) for p in self._COMPILED_POSIX_PREFIXES):
			return True
