...
class ExecutionSafetyManager:
	...
	_COMPILED_SENSITIVE_POSIX = tuple(re.compile(p, re.IGNORECASE) for p in _SENSITIVE_POSIX_PREFIXES)
	...
	_COMPILED_DESTRUCTIVE = tuple(re.compile(p) for p in _DESTRUCTIVE_PATTERNS)
	...
	_COMPILED_SHELL = tuple(re.compile(p) for p in _SHELL_PATTERNS)

	_POSIX_SYSTEM_PREFIXES_ABS = tuple(re.compile(p, re.IGNORECASE) for p in [
		r"/etc/\w+", r"/tmp/\w+", r"/var/\w+", r"/usr/\w+", r"/root/\w+",
		r"/home/\w+/", r"/proc/\w+", r"/sys/\w+", r"/dev/\w+", r"/boot/\w+",
		r"/opt/\w+", r"/mnt/\w+", r"/media/\w+"
	])
	_WIN_DRIVE_PATTERN = re.compile(r"[a-z]:[\\/]", re.IGNORECASE)
	_POSIX_QUOTE_PATTERN = re.compile(r"""["']/[^"'\s]""")
	_OPEN_ARGS_PATTERN = re.compile(r"open\s*\(\s*([\"'][^\"']+[\"'])", re.IGNORECASE)
	_OPEN_PATH_WIN_PATTERN = re.compile(r"[a-zA-Z]:[\\/]")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._WIN_DRIVE_PATTERN.search(code):
			return True

		# Quoted POSIX absolute path: '/...' or "/..."
		if self._POSIX_QUOTE_PATTERN.search(code):
			return True

		# Unquoted well-known POSIX system directory prefixes
		if any(p.search(code) for p in self._POSIX_SYSTEM_PREFIXES_ABS):
			return True
		...
		for arg in open_args:
			path = arg.strip("'\"")
			if path.startswith("/") or self._OPEN_PATH_WIN_PATTERN.match(path):
				return True
	...
	def _is_sensitive_posix_path(self, code: str) -> bool:
		...
		return any(p.search(code) for p in self._COMPILED_SENSITIVE_POSIX)
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		# =========================
		# AST BLOCK (Moved down as it's more expensive, but before regex destruct to preserve tests)
		# =========================
		...
		# filesystem deletes.
		# =========================
		if any(p.search(code_lower) for p in self._COMPILED_DESTRUCTIVE):
		...
		# =========================
		if any(p.search(code_lower) for p in self._COMPILED_SHELL):
	...
	def is_dangerous_operation(self, code: str) -> bool:
		...
		if not code or not code.strip():
			return False
		code_lower = code.lower()
		return any(p.search(code_lower) for p in self._COMPILED_DESTRUCTIVE)
