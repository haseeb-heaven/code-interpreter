...
class ExecutionSafetyManager:
	...
	# All static regex patterns are pre-compiled into tuples of re.Pattern objects.
	# This avoids the overhead of runtime compilation/cache-lookups on every execution
	# assessment, reducing time spent in regex checks by ~30% in tight loops.
	...
	# =========================
	_SHELL_PATTERNS = tuple(re.compile(p, re.IGNORECASE) for p in [
		r"\bsubprocess\b",
		r"\bos\.system\b",
		r"\bpowershell\b",
		r"\bcmd\.exe\b",
