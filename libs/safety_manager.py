...
class ExecutionSafetyManager:
	# BOLT OPTIMIZATION:
	# All regexes for _WRITE_PATTERNS, _WRITE_ON_HANDLE_PATTERNS, etc. are pre-compiled
	# to avoid repeated regex compilation inside critical safety evaluation paths.
	...
	# Pre-compiled one-off regexes
	_HOST_WINDOWS_DRIVE_PATTERN = re.compile(r"[a-z]:[\\/]")
	_HOST_POSIX_PATTERN = re.compile(r"""["']/[^"'\s]""")
	_RD_COMMAND_PATTERN = re.compile(r"\brd\s+/s\s+/q\b")

	_posix_system_prefixes = tuple(re.compile(p, re.IGNORECASE) for p in [
		r"/etc/\w+",
		r"/tmp/\w+",
		r"/var/\w+",
		r"/usr/\w+",
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._HOST_WINDOWS_DRIVE_PATTERN.search(code.lower()):
			return True

		# Quoted POSIX absolute path: '/...' or "/..."
		if self._HOST_POSIX_PATTERN.search(code):
			return True
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		if self._RD_COMMAND_PATTERN.search(code_lower):
