...
class ExecutionSafetyManager:
	...
	_RE_WIN_DRIVE_LOWER = re.compile(r"[a-z]:[\\/]")
	_RE_POSIX_ABS_QUOTED = re.compile(r"""["']/[^"'\s]""")
	_RE_OPEN_ARGS = re.compile(r"open\s*\(\s*([\"'][^\"']+[\"'])", re.IGNORECASE)
	_RE_WIN_DRIVE_ANY = re.compile(r"[a-zA-Z]:[\\/]")
	_RE_RD_S_Q = re.compile(r"\brd\s+/s\s+/q\b")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._RE_WIN_DRIVE_LOWER.search(code.lower()):
			return True

		# Quoted POSIX absolute path: '/...' or "/..."
		if self._RE_POSIX_ABS_QUOTED.search(code):
			return True
		...
		open_args = self._RE_OPEN_ARGS.findall(code)
		for arg in open_args:
			path = arg.strip("'\"")
			if path.startswith("/") or self._RE_WIN_DRIVE_ANY.match(path):
				return True
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		if self._RE_RD_S_Q.search(code_lower):
