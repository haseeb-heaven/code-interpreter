...
class ExecutionSafetyManager:
	...
	# BOLT OPTIMIZATION: Pre-compiling regexes as tuples avoids re cache lookup overhead in tight execution loops
	...
	# Pre-compiled absolute path checks for _is_host_absolute_path optimization
	_WIN_DRIVE_PATTERN = re.compile(r"[a-z]:[\\/]", re.IGNORECASE)
	_POSIX_ABS_PATTERN = re.compile(r"""["']/[^"'\s]""")
	...
	_RD_SQ_PATTERN = re.compile(r"\brd\s+/s\s+/q\b")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if self._POSIX_ABS_PATTERN.search(code):
			return True
	...
	def assess_execution(self, code: str, mode: str) -> Decision:
		...
		if self._RD_SQ_PATTERN.search(code_lower):
