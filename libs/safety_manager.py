...
class ExecutionSafetyManager:
	...
	_COMPILED_HOST_ABSOLUTE_POSIX_PREFIXES = tuple(re.compile(p, re.IGNORECASE) for p in [
		r"/etc/\w+",
		r"/tmp/\w+",
		r"/var/\w+",
		r"/usr/\w+",
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		if any(p.search(code) for p in self._COMPILED_HOST_ABSOLUTE_POSIX_PREFIXES):
			return True
