...
class ExecutionSafetyManager:
	...
	# Note: In Python 3 class bodies, list comprehensions do not have access to the class scope.
	# We use generator expressions converted to tuples to avoid NameError.
	...
	_COMPILED_WINDOWS_DRIVE_MATCH = re.compile(r"[a-zA-Z]:[\\/]")
	...
	def _is_host_absolute_path(self, code: str) -> bool:
		...
		for arg in open_args:
			path = arg.strip("'\"")
			if path.startswith("/") or self._COMPILED_WINDOWS_DRIVE_MATCH.match(path):
				return True
