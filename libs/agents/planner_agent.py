...
class PlannerAgent(BaseAgent):
	def __init__(self, model_router, logger, tool_registry=None):
		super().__init__(model_router, logger)
		self.tool_registry = tool_registry or getattr(getattr(model_router, "interp", None), "tool_registry", None)

	def _build_prompt(self, context: AgentContext) -> str:
		schemas = self.tool_registry.list_tools() if self.tool_registry else []
		return (
			f"Task: {context.task}\n"
	...
	def _apply_plan_data(self, context: AgentContext, plan_data: dict) -> None:
		...
		selected_tools = plan_data.get("tools") or plan_data.get("selected_tools")
		if selected_tools is not None:
			context.metadata["tools"] = selected_tools
	...
	def run(self, context: AgentContext) -> AgentContext:
		...
		try:
			...
			self._apply_plan_data(context, plan_data)
